// src/constants.js

// Firestore collection names
export const USERS = 'users';
export const CLASSES = 'classes';
export const ENROLLMENTS = 'enrollments';
export const SHIFTS = 'shifts';
export const TIMECLOCK = 'timeClock';
export const WAIVERS = 'waivers';

// Role and scheduling constants
export const INSTRUCTOR_ROLES = ['Lead Instructor', 'Assisting Instructor', 'OEC Instructor in Training', 'Instructor Trainer', 'Instructor of Record'];
export const SUPPORT_ROLES = ['Aid Room Patroller', 'Alpine Patroller', 'Professional Patroller', 'Safety Patroller', 'OEC Safety Patroller', 'Mountain Host', 'Other'];
export const PATROL_ADMIN_ROLES = ['Patrol Shift Leader', 'Assistant Patrol Shift Leader', 'Patrol Leadership'];
export const PATROL_LEADER_ROLES = ['Patrol Shift Leader', 'Assistant Patrol Shift Leader', 'Patrol Leadership'];
export const PATROL_ROLES = [
    'Paramedic/Advanced Provider',
    'MEDIC Alpine Patroller',
    'Alpine Patroller',
    'Aid Room Patroller',
    'Safety Patroller',
    'Mountain Host',
    'Guest Patroller',
    'Patrol Administration',
    'Candidate'
];
export const PATROLS = [
    'Patrol #1', 
    'Patrol #2', 
    'Patrol #3',
    'Patrol #1 (Night Chair)', 
    'Patrol #2 (Night Chair)', 
    'Patrol #3 (Night Chair)',
    'Patrol Administration (Not Assigned)',
    'Candidate (Not Assigned)',
    'Midweek (Not Assigned)',
    'Holiday (Not Assigned)'
];
export const MOUNTAIN_AREAS = [
    'Roaming/All Mountain',
    'Base Area/Plaza',
    'Aid Room',
    'Safety Patrol',
    'Mountain Host',
    'Mountain Top',
    'Forest Queen',
    'OEC Training',
    'OET Training',
    'AVI/MTR Training'
];
export const SHIFT_TYPES = ['Full Day', 'AM Half Day (Night Chair)', 'PM Half Day (Night Chair)', 'Night Chair'];

// List of NSP Patrols and Agencies
export const NSP_PATROLS = [
    "Crystal Mountain - Pro Patrol",
    "Crystal Mountain - Volunteer Patrol",
    "49 Degrees North",
    "705 Backcountry Nordic Ski Patrol",
    "705 Backcountry Ski Patrol",
    "Anthony Lakes",
    "Bald Mountain",
    "Bogus Basin",
    "Bogus Basin Pro Patrol",
    "Cascade Backcountry",
    "Cottonwood Butte",
    "Ferguson Ridge",
    "Hurricane Ridge",
    "Lookout Pass",
    "Loup Loup",
    "Magic Mountain",
    "Mission Ridge",
    "Mount Baker",
    "Mount Rainier Nordic Patrol",
    "Mt. Ashland",
    "Mt. Bachelor",
    "Mt. Hood Ski Patrol",
    "Payette Lakes",
    "Payette Lakes Nordic",
    "Santiam Pass",
    "Silver Mountain",
    "Ski Bluewood",
    "Snoqualmie Summit Alpental",
    "Snoqualmie Summit Central",
    "Snoqualmie Summit East (Hyak)",
    "Snoqualmie Summit West",
    "Snowhaven",
    "Soldier Mountain",
    "Soldier Mountain Pro Patrol",
    "Warner Canyon",
    "White Pass",
    "Willamette Backcountry",
    "Willamette Pass"
];

// App ID - using the dynamic version for compatibility with the environment
export const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
