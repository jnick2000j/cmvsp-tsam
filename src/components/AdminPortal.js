...
const AdminPortal = ({ currentUser, stations, classes, allUsers, setConfirmAction, waivers, branding, onBrandingUpdate }) => {
    ...
    // --- FIX START ---
    // This effect runs only once when the AdminPortal component is first loaded.
    // It ensures that all modals are in their default closed state.
    useEffect(() => {
    ...
    const instructors = useMemo(() => allUsers.filter(u => INSTRUCTOR_ROLES.includes(u.trainingRole) || u.isAdmin), [allUsers]);
    ...
    const filteredUsers = allUsers.filter(user => `${user.firstName} ${user.lastName}`.toLowerCase().includes(searchTerm.toLowerCase()) || user.email.toLowerCase().includes(searchTerm.toLowerCase()) || user.trainingRole.toLowerCase().includes(searchTerm.toLowerCase()));
                ...
                {adminView === 'users' && (
                    <>
                        ...
                        <div className="mt-6 flow-root"><div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8"><div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8"><div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">{filteredUsers.map((user) => (<tr key={user.id}><td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6"><div className="font-medium text-gray-900">{user.firstName} {user.lastName}</div><div className="text-gray-500">{user.email}</div></td><td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><div className="flex items-center gap-2"><span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${user.isAdmin ? 'bg-red-100 text-red-800' : INSTRUCTOR_ROLES.includes(user.trainingRole) ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>{user.isAdmin ? 'Admin' : user.trainingRole}</span>{INSTRUCTOR_ROLES.includes(user.trainingRole) && !user.isApproved && <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-yellow-100 text-yellow-800">Pending</span>}</div></td><td className="px-3 py-4 text-sm text-gray-500">{INSTRUCTOR_ROLES.includes(user.trainingRole) && (Object.keys(user.assignments || {}).length > 0 ? Object.keys(user.assignments).map(sId => { const station = stations.find(s => s.id === sId); if (!station) return null; const course = classes.find(c => c.id === station.classId); return `${station.name} (${course ? course.name : '...'})`; }).filter(Boolean).join(', ') : <span className="text-gray-400 italic">None</span>)}</td><td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6"><div className="flex items-center justify-end gap-2">{INSTRUCTOR_ROLES.includes(user.trainingRole) && !user.isApproved && <button onClick={() => handleApproveInstructor(user.id)} className="text-green-600 hover:text-green-900"><Check className="h-5 w-5"/></button>}<button onClick={() => handleEditUser(user)} className="text-indigo-600 hover:text-indigo-900"><Edit className="h-5 w-5" /></button><button onClick={() => handleDeactivateUserClick(user.id, `${user.firstName} ${user.lastName}`)} className="text-red-600 hover:text-red-900 disabled:text-gray-300" disabled={user.id === currentUser.uid}><Trash2 className="h-5 w-5" /></button></div></td></tr>))}</tbody></table>
                        </div></div></div></div>
                    </>
                )}
                {adminView === 'classes' && (
