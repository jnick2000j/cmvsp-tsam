// src/components/SupportAttendance.js
...
const SupportAttendance = ({ classes, allUsers, attendanceRecords, stations }) => {
    ...
    const supportPersonnel = useMemo(() => {
        return allUsers.filter(u => SUPPORT_ROLES.includes(u.role));
    }, [allUsers]);
    ...
    const filteredAttendance = useMemo(() => {
        ...
        let records = attendanceRecords.filter(rec => {
            ...
            const isSupportRecord = supportPersonnel.some(sp => sp.id === rec.userId);
            return isForThisClass && isSupportRecord;
        });

        if (searchTerm) {
            const lowercasedFilter = searchTerm.toLowerCase();
            records = records.filter(rec => {
                const user = supportPersonnel.find(u => u.id === rec.userId);
                return user && `${user.firstName} ${user.lastName}`.toLowerCase().includes(lowercasedFilter);
            });
        }

        return records;
    }, [selectedClassId, searchTerm, attendanceRecords, stations, supportPersonnel]);
    ...
    const downloadPdf = () => {
        ...
        const doc = new jsPDF();
        doc.text(`Support Personnel Attendance for: ${selectedClass.name}`, 14, 16);

        const tableColumn = ["Name", "Station", "Check-in Time", "Check-out Time", "Duration (min)"];
        const tableRows = [];

        filteredAttendance.forEach(record => {
            const user = supportPersonnel.find(u => u.id === record.userId);
        ...
        doc.save(`support_attendance_${selectedClass.name.replace(/\s+/g, '_')}.pdf`);
    };

    return (
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">View Support Personnel Attendance</h2>
                <button
                    onClick={downloadPdf}
                        ...
                        {selectedClassId ? filteredAttendance.map(record => {
                            ...
                            return (
                                <tr key={record.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{station ? station.name : 'N/A'}</td>
                            ...
                            <tr>
                                <td colSpan="5" className="px-6 py-12 text-center text-sm text-gray-500">
                                    Please select a class to view support attendance.
                                </td>
                            </tr>
                        )}
                    </tbody>
...
export default SupportAttendance;