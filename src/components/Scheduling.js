...
const Scheduling = ({ user, allUsers, shifts, timeClockEntries, onSignUp, onRequestSwap }) => {
    ...
    const shiftHistory = useMemo(() => {
        const history = myScheduledShifts.map(shift => {
            const entry = timeClockEntries.find(e => e.userId === user.id && new Date(e.clockInTime.seconds * 1000).toDateString() === new Date(shift.date).toDateString());
            return {
                ...shift,
                status: entry ? 'Attended' : 'Missed',
                clockIn: entry ? new Date(entry.clockInTime.seconds * 1000).toLocaleTimeString() : 'N/A',
                clockOut: entry && entry.clockOutTime ? new Date(entry.clockOutTime.seconds * 1000).toLocaleTimeString() : 'N/A'
            };
        });
        return history;
